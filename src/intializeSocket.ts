...
io.on("connection", (socket) => {
  socket.on("join_room", (data) => {
    ...
    console.log(
      `[${socket.id}]: has entered in room ${socketToRoom[socket.id]}`
    );
  ...
  socket.on("sendMessage", (data) => {
    ...
    console.log(`[${socket.id}] Sending message to room: ${roomID}`, {
      user,
      text,
      time,
    });
    ...
    if (roomID) {
      ...
      socket.to(roomID).emit("receiveMessage", { user, text, time });
    }
  });

  //Important to be check.
