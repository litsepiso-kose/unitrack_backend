...
import { generateOtp } from "./user_otp.service.js";
...
class UserService {
  ...
  async setUserVoiceLanguage(voiceLanguage: String, userId: String): Promise<Boolean> {
    try {
      await UserModel.findByIdAndUpdate(userId, { voiceLanguage })
      return true
    } catch (error) {
      console.log(error)
      return false
  ...
  async forgotPassword(email: String): Promise<boolean> {
    ...
    await this.sendPasswordResetToken(email);
